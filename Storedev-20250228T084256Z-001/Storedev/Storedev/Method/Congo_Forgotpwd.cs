...
{
    public class Congo_Forgotpwd 
    {
        protected IWebDriver driver;
        private IJavaScriptExecutor js;

        public Congo_Forgotpwd(IWebDriver driver)
        {
            this.driver = driver;
        ...
        {
            driver.FindElement(Congo_Locators.clickonforgotbutton).Click();
            Thread.Sleep(2000);
        }
        public void email_fill(string email)
        {
            driver.FindElement(Congo_Locators.fill_email).SendKeys(email);
            js.ExecuteScript("window.scrollBy(0, 150);");
            Thread.Sleep(30000);
        }
        public void clckbutton()
        {
            driver.FindElement(Congo_Locators.click_on_button).Click();
            Thread.Sleep(2000);
        }
        public void otp(string email)
        {
            ...
            Thread.Sleep(2000);
            driver.FindElement(Congo_Locators.Eneter_verificationcode).SendKeys(verificationCode);
            Thread.Sleep(500);
        }
        public void new_pass(string pass)
        {
            driver.FindElement(Congo_Locators.Eneter_Newpassword).SendKeys(pass);
            Thread.Sleep(500);
        }
        public void cnf_pass(string pass)
        {
            driver.FindElement(Congo_Locators.Eneter_cnfpassword).SendKeys(pass);
            Thread.Sleep(500);
        }
        public void clckbutton1()
        {
            driver.FindElement(Congo_Locators.click_on_submittbutton).Click();
            Thread.Sleep(2000);
        }
